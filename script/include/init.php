...
function sb_component_chat() {
    sb_js_global();
    sb_css();
    $header_headline = sb_get_setting('header-headline', 'Welcome');
    $header_message = sb_get_setting('header-msg','We are an experienced team that provides fast and accurate answers to your questions.');
    ...
    sb_custom_apps_load_js_css_files(false);
    ?>
                ...
                <div class="sb-header sb-header-main sb-header-type-<?php echo $header_type ?>" <?php echo $background ? 'style="background-image: url(' . $background . ')"' : '' ?>>
                    ...
                    <div class="sb-content">
                        ...
                        if ($header_type == 'agents') {
                            $agents = sb_db_get('SELECT id, first_name, profile_image FROM sb_users WHERE user_type = "agent"' . (sb_get_setting('header-exclude-admins') ? '' : ' OR user_type = "admin"') . ' ORDER BY last_activity DESC LIMIT 3', false);
                            $code = '';
                            for ($i = 0; $i < count($agents); $i++) {
                                $code .= '<div data-id="' . $agents[$i]['id'] . '"><span>' . $agents[$i]['first_name'] . '</span><img src="' . $agents[$i]['profile_image'] . '" loading="lazy" alt="" /></div>';
                            }
...
function sb_cross_site_init() {
    if (defined('SB_CROSS_DOMAIN') && defined('SB_CROSS_DOMAIN_URL') && SB_CROSS_DOMAIN) {
        ...
        for ($i = 0; $i < count($domains); $i++) {
            $domain = $domains[$i];
            if (!$current_domain || str_contains($domain, $current_domain)) {
