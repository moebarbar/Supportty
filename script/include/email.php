 ...
 * ==========================================================
 *
 * Email functions file. © 2017-2026 board.support. All rights reserved.
 *
 * ----------------------------------------------------------
 * EMAIL
 * ----------------------------------------------------------
...
function sb_email_create($recipient_id, $sender_name, $sender_profile_image, $message, $attachments = [], $department = false, $conversation_id = false, $cc = false) {
    ...
    } else if ($recipient_id == -1 || $recipient_id == 'agents' || str_contains($recipient_id, 'department-')) {
        if ($department) {
            $recipient_id = 'department-' . $department;
        }
        $agents = sb_db_get('SELECT id, first_name, last_name, email FROM sb_users WHERE id IN (' . sb_get_notification_agent_ids($recipient_id, true) . ')', false);
...
function sb_email_get_subject_and_body($recipient_user_type, $recipient_id = false, $conversation_id = false) {
    ...
    if (empty($body) && defined($is_agent ? 'SB_CLOUD_EMAIL_BODY_AGENTS' : 'SB_CLOUD_EMAIL_BODY_USERS')) {
        ...
        $body = sb_email_get_template($body, is_numeric($recipient_id) ? $recipient_id : false);
    }
...
function sb_email_send($to, $subject, $body, $sender_suffix = '', $cc = false, $reply_to = false, $user_id = false) {
    ...
    if ($user_id) {
        $body = sb_email_get_template($body, $user_id);
    }
    if ($host) {
        ...
        if ($reply_to && strpos($reply_to, '@') && $reply_to != $settings['email-server-from']) {
            $mail->addReplyTo($reply_to, sb_isset($settings, 'email-sender-name', '') . $sender_suffix);
        }
        if (sb_is_cloud()) {
    ...
    for ($j = 0; $j < count($settings_repeater); $j++) {
        ...
        if ($settings && !empty($settings['email-piping-active'])) {
            ...
            if ($inbox) {
                ...
                if ($emails) {
                    $department_id = sb_isset($settings, 'email-piping-department'); // Deprecated. Replace with: sb_isset($settings, ['email-piping-department', 0])
                    $department_id = is_array($department_id) ? $department_id[0] : $department_id; // Deprecated
                    $tags = sb_isset($settings, ['email-piping-tags', 0]);
                    ...
                    foreach ($emails as $email_number) {
                        ...
                        if (!$overview || (!isset($overview->senderaddress) && !isset($overview->from))) {
                            ...
                            $toaddress = isset($overview->toaddress) ? $overview->toaddress : '';
                                ...
                                if ($sender && ($follow_up || !sb_is_agent($sender))) {
                                    ...
                                    if (!empty($message)) {
                                        ...
                                        if (!$follow_up) {
                                            ...
                                            $conversation_id = empty($user_conversations) ? sb_isset(sb_new_conversation($sender['id'], 2, $subject, $department_id, -1, 'em', $cc, $j, false, $tags), 'details', [])['id'] : $user_conversations[0]['conversation_id'];
                                        }
...
function sb_subscribe_email($email) {
    $settings = sb_get_multilingual_setting('emails', 'email-subscribe', sb_get_user_language(sb_get_active_user_ID()));
    $subject = $settings['email-subscribe-subject'];
    $content = $settings['email-subscribe-content'];
    if ($settings && !empty($subject) && !empty($content)) {
        return sb_email_send($email, sb_merge_fields($subject), sb_merge_fields($content), '', false, false, sb_get_active_user_ID());
    }
    return false;
}

function sb_email_get_template($content, $user_id = false) {
    $language = $user_id ? sb_get_user_language($user_id) : 'en';
    $template = sb_get_multilingual_setting('emails', 'email-template', $language);
    if (empty($template)) {
        $template = '{content}';
    }
    ...
    if ($settings && $settings['newsletter-active']) {
        ...
        switch ($settings['newsletter-service']) {
            ...
            case 'sendfox':
                ...
                break;
            case 'klaviyo':
                $url = 'https://a.klaviyo.com/client/subscriptions?company_id=' . $key;
                $post_fields = ['type' => 'subscription', 'attributes' => ['profile' => ['data' => ['type' => 'profile', 'attributes' => ['email' => $email]]], 'custom_source' => 'Website Chat'], 'relationships' => ['list' => ['data' => ['type' => 'list', 'id' => $list_id]]]];
                break;
            case 'acumbamail':
                $url = 'https://acumbamail.com/api/1/addSubscriber/?auth_token=' . $key . '&list_id=' . $list_id . '&merge_fields[email]=' . $email . '&merge_fields[name]=' . urlencode(sb_get_user_name(['first_name' => $first_name, 'last_name' => $last_name]));
                break;
        }
        if ($url) {
...
function sb_rating_email() {
    $history = sb_get_external_setting('rating-email-history', []);
    $history_new = [];
    $conversations = sb_db_get('SELECT c.id, c.user_id FROM sb_conversations c LEFT JOIN sb_messages m ON m.conversation_id = c.id AND m.creation_time > "' . sb_gmt_now(864000) . '" AND m.payload LIKE \'%{"rating":%\' WHERE c.status_code = 3 AND m.id IS NULL', false);
    $response = [];
    $subject = sb_get_multi_setting('rating-message', 'rating-message-title', 'Rate your experience');
    $message = sb_get_multi_setting('rating-message', 'rating-email-body', 'How satisfied are you with the support you received?');
    $redirect_url = sb_get_multi_setting('rating-message', 'rating-redirect');
    foreach ($conversations as $conversation) {
        $conversation_id = $conversation['id'];
        $user_id = $conversation['user_id'];
        if (in_array($conversation_id, $history)) {
            array_push($history_new, $conversation_id);
            continue;
        }
        $user = sb_get_user($user_id);
        $language = sb_get_user_language($user_id);
        if (empty($user) || empty($user['email']) || sb_is_agent($user)) {
            continue;
        }
        $code = '<div style="display:flex;margin-top: 30px;align-items:center;justify-content:flex-start;border:1px solid #ccc;border-radius:20px;padding:10px 20px;background:none;width:fit-content;font-family:sans-serif;">
          <span style="font-weight:600;padding-right:30px;">' . sb_t('Rate and review', $language) . '</span>
          <div style="display:flex;align-items:center;gap:20px;">
            <a href="' . $redirect_url . '?rating=1&conversation=' . $conversation_id . '" style="display:flex;align-items:center;cursor:pointer;text-decoration:none;color:#222;margin-right:15px;font-weight:600;">
              <img src="' . SB_URL . '/media/like.png" style="width:20px;height:20px;margin-right: 10px;" alt="Like">
              <span style="color:#1a9260">' . sb_t('Helpful', $language) . '</span>
            </a>
            <a href="' . $redirect_url . '?rating=-1&conversation=' . $conversation_id . '" style="display:flex;align-items:center;cursor:pointer;text-decoration:none;color:#222;font-weight:600;">
              <img src="' . SB_URL . '/media/dislike.png" style="width:20px;height:20px;margin-right: 10px;" alt="Dislike">
              <span style="color:#ca3434">' . sb_t('Not helpful', $language) . '</span>
            </a>
          </div>
        </div>';
        $response_ = sb_email_send($user['email'], sb_t($subject, $language), sb_merge_fields(sb_t($message, $language) . $code), '', false, false, $user_id);
        if ($response_ === true) {
            array_push($history_new, $conversation_id);
            $response[$conversation_id] = true;
        } else {
            $response[$conversation_id] = $response_;
        }
    }
    sb_save_external_setting('rating-email-history', $history_new);
    return $response;
}
