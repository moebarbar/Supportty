 ...
 * ==========================================================
 *
 * AJAX functions. This file must be executed only via AJAX. © 2017-2026 board.support. All rights reserved.
 *
 */
...
function sb_ajax_execute() {
    ...
    switch ($function) {
        case 'login-link':
            return sb_json_response(sb_login_link($_POST['email'], $_POST['url']));
        case 'merge-users':
            return sb_json_response(sb_merge_users($_POST['user_id_1'], $_POST['user_id_2']));
        case 'calendar-get-available-slots':
            return sb_json_response(sb_integrations_calendar_get_available_slots($_POST['date'], sb_post('max_days')));
        case 'calendar-create-booking':
            return sb_json_response(sb_integrations_calendar_create_booking($_POST['start_date'], $_POST['user_id']));
        case 'calendar-panel':
            return sb_json_response(sb_integrations_calendar_panel($_POST['user_id']));
        case 'calendar-cancel-booking':
            return sb_json_response(sb_integrations_calendar_cancel_booking($_POST['booking_id']));
        case 'get-user-by':
        ...
        case 'add-saved-reply':
            return sb_json_response(sb_add_saved_reply($_POST['name'], $_POST['text']));
        case 'save-settings':
        ...
        case 'close-message':
            return sb_json_response(sb_close_message($_POST['conversation_id']));
        case 'rating-message':
            return sb_json_response(sb_rating_message($_POST['conversation_id']));
        ...
        case 'send-custom-email':
            return sb_json_response(sb_email_send($_POST['to'], $_POST['subject'], sb_email_get_template(sb_merge_fields(defined('SB_DIALOGFLOW') ? sb_google_translate_auto($_POST['message'], sb_post('recipient_id')) : $_POST['message']), sb_post('recipient_id'))));
        ...
        case 'google-get-token':
            return sb_json_response(sb_google_get_token());
        case 'human-takeover':
            return sb_json_response(sb_dialogflow_human_takeover($_POST['conversation_id'], sb_post('auto_messages'), sb_post('is_confirmation_message', true)));
        case 'set-rating':
        ...
        case 'reports':
            return sb_json_response(sb_reports_get($_POST['name'], sb_post('date_start'), sb_post('date_end'), sb_post('timezone')));
        case 'reports-get-code':
            return sb_json_response(sb_reports_get_code($_POST['name'], sb_post('date_start'), sb_post('date_end'), sb_post('timezone')));
        case 'reports-update':
        ...
        case 'messenger-get-posts':
            return sb_json_response(sb_messenger_get_posts());
        case 'messenger-unsubscribe':
            return sb_json_response(sb_messenger_unsubscribe());
        case 'messenger-send-message':
            return sb_json_response(sb_messenger_send_message($_POST['psid'], $_POST['facebook_page_id'], sb_post('message', ''), sb_post('attachments', []), sb_post('metadata', []), sb_post('message_id'), sb_post('reply_to')));
        case 'messenger-send-comment':
            return sb_json_response(sb_messenger_send_comment($_POST['comment_id'], $_POST['facebook_page_id'], $_POST['message'], $_POST['message_id'], sb_post('is_instagram', true)));
        case 'whatsapp-send-message':
            return sb_json_response(sb_whatsapp_send_message($_POST['to'], sb_post('message', ''), sb_post('attachments', []), sb_post('phone_id'), sb_post('reply_to'), sb_post('message_id')));
        ...
        case 'whatsapp-call-answer':
            return sb_json_response(sb_whatsapp_call_answer($_POST['phone_number_id'], $_POST['call_id'], $_POST['sdp_answer'], sb_post('is_accept')));
        case 'whatsapp-call-decline':
            return sb_json_response(sb_whatsapp_call_decline($_POST['phone_number_id'], $_POST['call_id'], sb_post('is_terminate')));
        case 'whatsapp-call-start':
            return sb_json_response(sb_whatsapp_call_start($_POST['sdp'], $_POST['user_id'], sb_post('phone_number_id', '')));
        case 'whatsapp-call-save-audio':
            return sb_json_response(sb_whatsapp_call_save_audio($_POST['conversation_id'], $_POST['audio_file_url'], sb_post('duration')));
        ...
        case 'telegram-send-message':
            return sb_json_response(sb_telegram_send_message($_POST['chat_id'], sb_post('message', ''), sb_post('attachments', []), sb_post('conversation_id'), sb_post('reply_to'), sb_post('message_id')));
        ...
        case 'send-sms':
            return sb_json_response(sb_send_sms(defined('SB_DIALOGFLOW') ? sb_google_translate_auto($_POST['message'], sb_post('recipient_id')) : $_POST['message'], $_POST['to'], sb_post('template', true), sb_post('conversation_id'), sb_post('attachments')));
        case 'direct-message':
        ...
        case 'google-translate':
            return sb_json_response(sb_google_translate($_POST['strings'], $_POST['language_code'], sb_post('message_ids'), sb_post('conversation_id')));
        case 'google-language-detection-update-user':
            return sb_json_response(sb_google_language_detection_update_user($_POST['string'], sb_post('user_id')));
        ...
        case 'verification-cookie':
            return sb_json_response(sb_verification_cookie($_POST['code'], $_POST['domain'], sb_post('user')));
        case 'on-close':
        ...
        case 'open-ai-rewrite-message':
            return sb_json_response(sb_open_ai_rewrite_message($_POST['message']));
        ...
        case 'open-ai-flows-get':
            return sb_json_response(sb_flows_get(sb_post('name'), false));
        ...
        case 'open-ai-audio-transcript':
            return sb_json_response(sb_open_ai_audio_transcript($_POST['url'], sb_post('message_id'), sb_post('language')));
        ...
        case 'get-tags':
            return sb_json_response(sb_get_setting('tags', []));
        case 'audio-clip':
...
function sb_security($function) {
    $security = [
        ...
        'admin' => ['open-ai-save-conversation-embeddings', 'open-ai-get-conversation-embeddings', 'import-users', 'messenger-unsubscribe', 'open-ai-flows-save', 'open-ai-playground-message', 'open-ai-get-information', 'open-ai-url-training', 'open-ai-file-training', 'open-ai-get-training-files', 'update-sw', 'open-ai-articles-training', 'open-ai-embeddings-delete', 'get-sitemap-urls', 'open-ai-source-file-to-paragraphs', 'open-ai-generate-embeddings', 'get-user-by', 'get-agent-department', 'upload-path', 'get-multi-setting', 'save-external-setting', 'get-external-setting', 'update-bot', 'get-last-message', 'delete-attachments', 'slack-channels', 'twitter-subscribe', 'whatsapp-360-synchronization', 'telegram-synchronization', 'viber-synchronization', 'import-settings', 'export-settings', 'updates-available', 'automations-save', 'path', 'reports-export', 'reports', 'get-report-code', 'aecommerce-sync-admins', 'aecommerce-sync-sellers', 'aecommerce-sync', 'whmcs-sync', 'whmcs-articles-sync', 'perfex-articles-sync', 'perfex-sync', 'woocommerce-get-session', 'woocommerce-get-attributes', 'woocommerce-get-taxonomies', 'delete-leads', 'system-requirements', 'save-settings', 'get-settings', 'get-all-settings', 'delete-user', 'delete-users', 'app-get-key', 'app-activation', 'app-disable', 'wp-sync'],
        'agent' => ['get-tags', 'open-ai-audio-transcript', 'messenger-get-posts', 'open-ai-rewrite-message', 'merge-users', 'rating-message', 'add-saved-reply', 'calendar-add-booking', 'calendar-get-available-slots', 'calendar-cancel-booking', 'calendar-panel', 'shopify-get-product-link', 'shopify-get-conversation-details', 'update-conversation-extra', 'generate-sitemap', 'open-ai-html-to-paragraphs', 'google-troubleshoot', 'open-ai-troubleshoot', 'whatsapp-clear-flows', 'assign-conversations-active-agent', 'init-articles-admin', 'data-scraping', 'opencart-order-details', 'opencart-panel', 'open-ai-get-qea-training', 'open-ai-qea-training', 'get-tags', 'add-user', 'get-html', 'envato', 'whatsapp-get-templates', 'automations-is-sent', 'logs', 'newsletter', 'get-last-agent-in-conversation', 'messaging-platforms-send-message', 'open-ai-user-expressions', 'whatsapp-send-template', 'martfury-sync', 'martfury-sync-sellers', 'martfury-get-conversation-details', 'zendesk-update-ticket', 'zendesk-create-ticket', 'zendesk-get-conversation-details', 'save-articles-categories', 'on-close', 'check-conversations-assignment', 'delete-file', 'ump-get-conversation-details', 'armember-get-conversation-details', 'count-conversations', 'reports-update', 'get-agents-ids', 'send-custom-email', 'get-users-with-details', 'direct-message', 'messenger-send-comment', 'messenger-send-message', 'wechat-send-message', 'whatsapp-send-message', 'telegram-send-message', 'viber-send-message', 'zalo-send-message', 'line-send-message', 'twitter-send-message', 'get-user-language', 'get-notes', 'add-note', 'update-note', 'delete-note', 'user-online', 'get-user-from-conversation', 'aecommerce-get-conversation-details', 'whmcs-get-conversation-details', 'woocommerce-get-order', 'woocommerce-coupon-delete-expired', 'woocommerce-coupon-check', 'woocommerce-coupon', 'woocommerce-is-in-stock', 'woocommerce-get-attribute-by-name', 'woocommerce-get-attribute-by-term', 'woocommerce-get-product-taxonomies', 'woocommerce-get-product-images', 'woocommerce-get-product-id-by-name', 'woocommerce-get-user-orders', 'woocommerce-get-product', 'woocommerce-get-customer', 'woocommerce-products-popup', 'woocommerce-search-products', 'woocommerce-get-products', 'woocommerce-get-data', 'is-agent-typing', 'close-message', 'count-users', 'get-users', 'get-new-users', 'get-online-users', 'search-users', 'get-conversations', 'get-new-conversations', 'search-conversations', 'csv-users', 'send-test-email', 'slack-users', 'clean-data', 'save-translations', 'get-rating', 'save-article', 'update', 'archive-slack-channels'],
        'user' => ['whatsapp-call-save-audio', 'whatsapp-call-start', 'whatsapp-call-answer', 'whatsapp-call-decline', 'shopify-cart-sync', 'open-ai-send-fallback-message', 'open-ai-flows-get', 'audio-to-text', 'update-tags', 'execute-bot-message', 'remove-email-cron', 'aws-s3', 'is-allowed-extension', 'translate-string', 'human-takeover', 'google-language-detection-update-user', 'google-translate', 'get-agents-in-conversation', 'update-conversation-agent', 'update-conversation-department', 'get-avatar', 'slack-presence', 'woocommerce-waiting-list', 'search-user-conversations', 'update-login', 'update-user', 'get-user', 'get-user-extra', 'update-user-to-lead', 'new-conversation', 'get-user-conversations', 'get-new-user-conversations', 'send-slack-message', 'slack-unarchive', 'update-message', 'delete-message', 'update-user-and-message', 'get-conversation', 'get-new-messages', 'set-rating', 'create-email', 'send-email']
    ];
    ...
    if ($active_user && isset($active_user['user_type'])) {
        ...
        // User check
        if (in_array($function, $security['user']) && (sb_is_agent($user_type) || ($user_id == $current_user_id && (empty($_POST['conversation_id']) || in_array($_POST['conversation_id'], array_column(sb_db_get('SELECT id FROM sb_conversations WHERE user_id = ' . $user_id, false), 'id')))))) {
            return true;
        }
