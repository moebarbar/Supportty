 ...
 * ==========================================================
 *
 * Users functions file. © 2017-2026 board.support. All rights reserved.
 *
 * -----------------------------------------------------------
 * LOGIN AND ACCOUNT
 * -----------------------------------------------------------
...
function sb_otp($email = false, $otp = false) {
    ...
    if ($email) {
        $language = sb_get_user_language(sb_get_active_user_ID());
        $response = sb_email_send($email, sb_get_multilingual_setting('email-otp', 'email-otp-subject', $language, 'One-Time Code'), str_replace('{code}', $otp, sb_get_multilingual_setting('email-otp', 'email-otp-content', $language, 'Your one-time code is {code}.')), '', false, false, sb_get_active_user_ID());
        return $response ? sb_encryption(json_encode([$otp, $email])) : false;
    }
    return false;
}

function sb_login_link($email, $url) {
    $user = $email ? sb_get_user_by('email', $email) : false;
    if ($user) {
        $language = sb_get_user_language($user['id']);
        return sb_email_send($email, sb_merge_fields(sb_get_multilingual_setting('email-login-link', 'email-login-link-subject', $language, 'Login link'), [sb_get_user_name($user)]), str_replace('{link}', $url . '?token=' . $user['token'], sb_merge_fields(sb_get_multilingual_setting('email-login-link', 'email-login-link-content', $language, 'Log in using the following link: {link}.'), [sb_get_user_name($user)])), '', false, false, $user['id']);
    }
    return false;
}
...
function sb_update_user($user_id, $settings, $settings_extra = [], $hash_password = true, $skip_otp = false) {
    ...
    if ($first_name && (strpos($profile_image, 'media/user.svg') || !$profile_image)) {
        if (!$profile_image) {
            $check = sb_isset(sb_db_get('SELECT profile_image FROM sb_users WHERE id = ' . $user_id), 'email');
            $check = empty($check) || strpos($check, 'media/user.svg');
        }
        if ($profile_image || $check) {
    ...
    if ($email) {
        ...
        if ($existing_user) {
            if (empty($settings['otp'])) {
                ...
                if (sb_is_agent($existing_user)) {
                    return 'admin-user-error';
                }
                if (!sb_is_agent()) {
                    sb_db_query('UPDATE sb_conversations SET user_id = ' . $existing_user['id'] . ' WHERE user_id = ' . sb_get_active_user_ID());
                }
                $response = sb_login(false, false, $existing_user['id'], $existing_user['token']);
                $response[0]['user_change'] = true;
                $response[0]['id'] = $existing_user['id'];
                return $response;
            }
        } else {
            $query .= ', email = "' . $email . '"';
    ...
    // Extra user details
    if ($active_user && $active_user['id'] == $user_id && (!function_exists('sb_open_ai_is_playground') || !sb_open_ai_is_playground())) {
        $user = sb_get_user($user_id);
        if ($user) {
...
function sb_update_user_and_message($user_id, $settings, $settings_extra = [], $message_id = false, $message = false, $payload = false, $skip_otp = false) {
    ...
    if (sb_is_validation_error($result) && ($result->code() == 'duplicate-email' || $result->code() == 'invalid-otp')) {
        return $result;
    }
    if ($message_id) {
        if ($message) {
    ...
    if (isset($user) && is_array($user)) {
        ...
        for ($i = 0; $i < count($user['details']); $i++) {
            if ($user['details'][$i]['slug'] == 'country') {
                $country_code = strtolower($user['details'][$i]['value']);
                if ($country_code && file_exists(SB_PATH . '/media/flags/' . $country_code . '.png')) {
                    $user['flag'] = $country_code . '.png';
                }
                break;
            }
        }
...
function sb_get_user_name($user = false) {
    ...
    return $name && substr(sb_isset($user, 'last_name', '-'), 0, 1) != '#' ? $name : sb_get_setting('visitor-default-name', $name);
}
...
function sb_current_url($user_id = false, $url = false) {
    if (!empty($user_id)) {
        if ($url === false) {
            return sb_get_user_extra($user_id, 'current_url');
        }
...
function sb_get_agents_ids($admins = true) {
    ...
    $SB_AGENT_IDS = sb_db_get('SELECT id FROM sb_users WHERE user_type = "agent"' . ($admins ? ' OR user_type = "admin"' : ''), false);
    for ($i = 0; $i < count($SB_AGENT_IDS); $i++) {
        $SB_AGENT_IDS[$i] = intval($SB_AGENT_IDS[$i]['id']);
    }
    return $SB_AGENT_IDS;
}
...
function sb_set_rating($conversation_id, $user_id, $rating, $message = false, $agent_id = false) {
    ...
    sb_db_query('DELETE FROM sb_messages WHERE id = (SELECT MAX(id) FROM sb_messages WHERE conversation_id = 8360 AND message LIKE "%[rating %")');
...
function sb_merge_users($user_id_1, $user_id_2) {
    ...
    if (sb_is_agent($user_1) || sb_is_agent($user_2)) {
        return sb_error('invalid-user-type', 'sb_merge_users', 'One of the users is an agent or admin.');
    }
...
function sb_queue_check_and_run($conversation_id, $department, $source = false) {
    if (sb_get_multi_setting('queue', 'queue-active')) {
        $continue = !defined('SB_DIALOGFLOW') || !sb_is_chatbot_active($source ? 'ig' : 'fb', $conversation_id) || !sb_dialogflow_is_human_takeover_active_setting();
        if (!$continue) {
