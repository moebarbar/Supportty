 ...
 * ==========================================================
 *
 * Settings functions file. © 2017-2026 board.support. All rights reserved.
 *
 * -----------------------------------------------------------
 * SETTINGS
 * -----------------------------------------------------------
 ...
 * 29. Get active custom apps
 * 30. Load JS and CSS custom app files
 *
 */
...
function sb_get_front_settings() {
    ...
    $return = [
        ...
        'header_name' => sb_get_multi_setting('header-name', 'header-name-active') ? sb_get_multi_setting('header-name', 'header-name-text', 'Hello, {user_name}!') : false,
        ...
        'bot_name' => sb_get_setting('bot-name', 'Chatbot'),
        'bot_image' => sb_get_setting('bot-image', SB_URL . '/media/user.png'),
        'bot_delay' => sb_get_setting('dialogflow-bot-delay', 2),
        'is_chatbot_active' => sb_is_chatbot_active(),
    ...
    if ($return['rating']) {
        ...
        $return['rating_title'] = sb_get_multi_setting('rating-message', 'rating-message-title', 'Rate your experience');
        $return['rating_text'] = sb_get_multi_setting('rating-message', 'rating-message-text', 'How satisfied are you with the support you received?');
    }
    if ($return['articles']) {
    ...
    if ($return['welcome']) {
        ...
        $return['welcome_delay'] = sb_get_multi_setting('welcome-message', 'welcome-delay', 2);
    ...
    if ($return['tickets']) {
        ...
        $return['tickets_help_text'] = sb_get_setting('tickets-help-text');
    ...
    if ($return['is_chatbot_active']) {
        $return['dialogflow_human_takeover'] = sb_dialogflow_is_human_takeover_active_setting();
...
function sb_js_admin() {
    ...
    $settings = [
        ...
        'chatbot_features' => sb_get_multi_setting('open-ai', 'open-ai-active') || sb_get_setting('ai-smart-reply'),
    ...
    $channel_labels = [];
    if (defined('SB_MESSENGER')) {
        $items = sb_get_setting('messenger-pages', []);
        if (is_array($items) && count($items) > 1) {
            foreach ($items as $item) {
                $channel_labels[$item['messenger-page-id']] = $item['messenger-page-name'];
                if (!empty($item['messenger-instagram-id'])) {
                    $channel_labels[$item['messenger-instagram-id']] = $item['messenger-page-name'];
                }
            }
        }
    }
    if (defined('SB_WHATSAPP')) {
        $items = sb_get_setting('whatsapp-cloud-numbers', []);
        if (is_array($items) && count($items) > 1) {
            foreach ($items as $item) {
                $channel_labels[$item['whatsapp-cloud-numbers-phone-id']] = $item['whatsapp-cloud-numbers-label'];
            }
        }
    }
    if (defined('SB_TELEGRAM')) {
        $items = sb_get_setting('telegram-numbers', []);
        if (is_array($items) && count($items) > 1) {
            foreach ($items as $item) {
                $channel_labels[$item['telegram-numbers-token']] = $item['telegram-numbers-label'];
            }
        }
    }
    $settings['channel_labels'] = $channel_labels;
    if (defined('SB_WOOCOMMERCE')) {
    ...
    if ($translations) {
        ...
        foreach ($custom_apps as $app) {
            $path_app = SB_PATH . '/apps/' . $app[0] . '/languages/admin/js/' . $language . '.json';
            if (file_exists($path_app)) {
                $app_translations = json_decode(file_get_contents($path_app), true);
                if ($app_translations) {
                    $translations = array_merge($translations, $app_translations);
                }
            }
        }
    }
...
function sb_get_block_setting($value) {
    switch ($value) {
        case 'privacy':
            ...
            return $settings && $settings['privacy-active'] ? ['title' => sb_rich_value($settings['privacy-title']), 'message' => sb_rich_value($settings['privacy-msg']), 'decline' => sb_rich_value($settings['privacy-msg-decline']), 'link' => $settings['privacy-link'], 'link-name' => sb_rich_value(sb_isset($settings, 'privacy-link-text', ''), false), 'btn-approve' => sb_rich_value(sb_isset($settings, 'privacy-btn-approve', 'Accept'), false), 'btn-decline' => sb_rich_value(sb_isset($settings, 'privacy-btn-decline', 'Decline'), false)] : false;
        case 'popup':
        ...
        case 'follow':
            ...
            return $settings && $settings['follow-active'] ? ['title' => sb_rich_value($settings['follow-title']), 'message' => sb_rich_value($settings['follow-msg'], false, true), 'sound' => $settings['follow-sound'], 'name' => $settings['follow-name'] ? 'true' : 'false', 'last-name' => sb_isset($settings, 'follow-last-name') ? 'true' : 'false', 'phone' => sb_isset($settings, 'follow-phone') ? 'true' : 'false', 'phone-required' => sb_isset($settings, 'follow-phone-required') ? 'true' : 'false', 'success' => sb_rich_value(sb_isset($settings, 'follow-success', 'Thanks! You\'ll be notified at {user_email}.')), 'placeholder' => sb_rich_value(sb_isset($settings, 'follow-placeholder', 'Email')), 'delay' => sb_isset($settings, 'follow-delay'), 'disable-office-hours' => sb_isset($settings, 'follow-disable-office-hours')] : false;
    }
    return false;
}
...
function sb_get_setting_code($setting) {
    if (isset($setting)) {
        ...
        $placeholder = empty($setting['placeholder']) ? '' : ' placeholder="' . sb_s($setting['placeholder']) . '"';
        switch ($type) {
            case 'color':
                $content .= '<input type="text"><i class="sb-close sb-icon-close"></i>';
                break;
            case 'select-ajax':
                $content .= '<input type="text" ' . $placeholder . ' readonly>';
                break;
            case 'text':
            ...
            case 'radio':
                ...
                break;
            case 'number':
                $content .= '<input type="number"' . $placeholder . '>' . (isset($setting['unit']) ? '<label>' . $setting['unit'] . '</label>' : '');
                break;
            case 'upload':
            ...
            case 'multi-input':
                $values = $setting['value'];
                for ($i = 0; $i < count($values); $i++) {
                    $setting_sub = $values[$i];
                    $sub_type = $setting_sub['type'];
                    $placeholder = empty($setting_sub['placeholder']) ? '' : ' placeholder="' . sb_($setting_sub['placeholder']) . '"';
                    $content .= '<div id="' . $setting_sub['id'] . '" data-type="' . $sub_type . '" class="multi-input-' . $sub_type . '"><label>' . sb_s($setting_sub['title'], $disable_translations) . sb_get_setting_code_help($setting_sub) . '</label>';
                    switch ($sub_type) {
                        case 'select-ajax':
                            $content .= '<input type="text" ' . $placeholder . ' readonly>';
                            break;
                        ...
                        case 'select':
                            ...
                            break;
                        case 'button':
                            $content .= '<a class="sb-btn" href="' . $setting_sub['button-url'] . '" target="_blank">' . sb_s($setting_sub['button-text'], $disable_translations) . '</a>';
                            break;
                        case 'select-checkbox':
                            $items = $setting_sub['value'];
                            $content .= '<input type="text" class="sb-select-checkbox-input" readonly><div class="sb-select-checkbox">';
                            foreach ($items as $item_) {
                                $content .= '<div class="multi-input-checkbox"><input id="' . $item_[0] . '" type="checkbox"><label>' . sb_s($item_[1], $disable_translations) . '</label></div>';
                            }
                            $content .= '</div>';
                            break;
                    }
                ...
                for ($i = 0; $i < count($setting['items']); $i++) {
                    ...
                    switch ($item['type']) {
                        ...
                        case 'select':
                            ...
                            break;
                        case 'select-ajax':
                            $content .= '<input data-id="' . $item['id'] . '" type="text" data-type="select-ajax" readonly>';
                            break;
                        case 'select-checkbox':
                            $items = $item['value'];
                            $content .= '<input type="text" class="sb-select-checkbox-input" data-type="select-checkbox" data-id="' . $item['id'] . '" readonly><div class="sb-select-checkbox">';
                            foreach ($items as $item_) {
                                $content .= '<div class="multi-input-checkbox"><input id="' . $item_[0] . '" type="checkbox"><label>' . sb_s($item_[1], $disable_translations) . '</label></div>';
                            }
                            $content .= '</div>';
            ...
            case 'timetable':
                ...
                for ($i = 0; $i < 7; $i++) {
                    $content .= '<div data-day="' . strtolower($days[$i]) . '"><label>' . sb_($days[$i]) . '</label><div><div>' . sb_('Closed') . '</div><span>' . sb_('To') . '</span><div>' . sb_('Closed') . '</div><span>' . sb_('And') . '</span><div>' . sb_('Closed') . '</div><span>' . sb_('To') . '</span><div>' . sb_('Closed') . '</div></div></div>';
                }
            ...
            case 'select-checkbox':
                $values = $setting['value'];
                $content .= '<select disabled><option></option></select><div class="sb-select-checkbox">';
                foreach ($values as $value_) {
                    $content .= '<div id="' . $value_['id'] . '" data-type="checkbox" class="multi-input-checkbox"><input type="checkbox"><label>' . sb_s($value_['title'], $disable_translations) . '</label></div>';
                }
                $content .= '</div>';
                break;
        }
...
function sb_delete_external_setting($name) {
    sb_db_query('DELETE FROM sb_settings WHERE name = "' . $name . '" LIMIT 1');
}
...
function sb_get_multilingual_setting($name, $sub_name, $language = false, $default = false) {
    ...
    $value = sb_isset(sb_get_external_setting($name), $sub_name, $default);
...
function sb_export_settings() {
    $setting_keys = ['automations', 'emails', 'rich-messages', 'settings', 'app-keys', 'articles', 'articles-categories', 'open-ai-intents-history', 'slack-channels'];
    $settings = [];
...
function sb_office_hours($range_start_gmt = false, $range_end_gmt = false) {
    $settings = sb_get_settings();
    $timetable = sb_isset($settings, 'timetable', [[]])[0];
    $utc_offset = floatval(sb_utc_offset());
    if (empty($range_start_gmt)) {
        $range_start_gmt = time();
    }
    $range_end_gmt = empty($range_end_gmt) || $range_end_gmt == 'false' ? $range_start_gmt : $range_end_gmt;
    $offset = $range_start_gmt - ($utc_offset * 3600);
    $start_day = strtolower(gmdate('l', $offset));
    $start_day_array = explode('-', gmdate('m-d-y', $offset));
    $start_day_array = [intval($start_day_array[0]), intval($start_day_array[1]), intval($start_day_array[2])];
    if (isset($timetable[$start_day]) && !empty($timetable[$start_day][0][0])) {
        $status = false;
        for ($i = 0; $i < 3; $i += 2) {
            if (!empty($timetable[$start_day][$i][0]) && $timetable[$start_day][$i][0] != 'closed') { // Deprecated. Remove  && $timetable[$start_day][$i][0] != 'closed'
                $start = explode(':', $timetable[$start_day][$i][0]);
                $end = explode(':', $timetable[$start_day][$i + 1][0]);
                $office_hours_start = gmmktime(intval($start[0]) + $utc_offset, intval($start[1]), 0, $start_day_array[0], $start_day_array[1], $start_day_array[2]);
                $office_hours_end = gmmktime(intval($end[0]) + $utc_offset, intval($end[1]), 0, $start_day_array[0], $start_day_array[1], $start_day_array[2]);
                if ($range_end_gmt >= $office_hours_start && $range_start_gmt <= $office_hours_end && $range_start_gmt >= $office_hours_start && $range_end_gmt <= $office_hours_end) {
                    $status = true;
                }
            }
        }
...
function sb_css($color_1 = false, $color_2 = false, $color_3 = false, $return = false) {
    ...
    if ($color_1) {
        $css .= '.sb-chat-btn, .sb-chat>div>.sb-header,.sb-chat .sb-dashboard>div>.sb-btn:hover,.sb-chat .sb-scroll-area .sb-header,.sb-input.sb-input-btn>div,div ul.sb-menu li:hover,.sb-popup.sb-emoji .sb-emoji-bar>div.sb-active,
...
function sb_update_sw($url) {
    ...
    for ($i = 0; $i < count($lines); $i++) {
        if (str_contains($lines[$i], 'importScripts')) {
...
function sb_add_saved_reply($name, $text) {
    if (empty($name) || empty($text)) {
        return false;
    }
    $settings = sb_get_all_settings();
    $saved_replies = sb_isset($settings, 'saved-replies', []);
    array_push($saved_replies[0], ['reply-name' => $name, 'reply-text' => $text]);
    $settings['saved-replies'] = $saved_replies;
    return sb_save_settings($settings);
}

function sb_custom_apps_get_active() {
    $custom_apps = [];
    if (defined('SB_CUSTOM_APPS') && !empty(SB_CUSTOM_APPS)) {
        foreach (SB_CUSTOM_APPS as $custom_app) {
            if (defined('SB_' . strtoupper($custom_app[0]))) {
                array_push($custom_apps, $custom_app);
            }
        }
    }
    return $custom_apps;
...
function sb_custom_apps_load_js_css_files($is_admin = true) {
    $code = '';
    $custom_apps = sb_custom_apps_get_active();
    $slug = $is_admin ? 'admin' : 'front';
    foreach ($custom_apps as $custom_app) {
        for ($i = 0; $i < 2; $i++) {
            $file_path = '/apps/' . $custom_app[0] . '/' . $slug . '.' . ($i ? 'js' : 'css');
            if (file_exists(SB_PATH . $file_path)) {
                if (!$i) {
                    $code .= '<link rel="stylesheet" href="' . SB_URL . $file_path . '?v=' . SB_VERSION . '" />';
                } else {
                    $code .= '<script src="' . SB_URL . $file_path . '?v=' . SB_VERSION . '"></script>';
                }
            }
        }
    }
 ...
 * 4. Return all articles categories
 * 5. Return a single article category
 * 6. Article search
 * 7. Article ratings
 * 8. Inits articles for the admin area
 * 9. Generates an excerpt of the article contents
 * 10. Return the article.php
 * 11. Return the article URL
 * 12. Return the articles page URL
 * 13. Checks if the article URL rewriting is enabled
 * 14. Return the article languages
 *
 */
...
function sb_save_article($article) {
    ...
    foreach ($matches[1] as $code) {
        $article_content = str_replace($code, htmlspecialchars(str_replace(['&lt;script', '&lt;/script'], ['<script', '</script'], $code)), $article_content);
    }

    // Nav creation
    $headings = [];
    if ($article_editor_js) {
        foreach ($article_editor_js['blocks'] as $block) {
            if ($block['type'] == 'header' && in_array($block['data']['level'], [2, 3])) {
                array_push($headings, [$block['data']['text'], $block['data']['level']]);
            }
        }
    }
    $article_content = preg_replace_callback('/<(h[23])>(.*?)<\/\1>/i', function ($matches) {
        $tag = $matches[1];
        $text = strip_tags($matches[2]);
        $id = sb_string_slug($text);
    ...
    if (!$article_id) {
        $response = sb_db_query('INSERT INTO sb_articles (title, content, editor_js, nav, link, category, parent_category, language, parent_id, slug, update_time) VALUES ("' . $article_title . '", "' . $article_content . '", "' . $article_editor_js . '", "' . sb_db_json_escape($headings) . '", "' . $article_link . '", "' . $article_category . '", "' . $article_parent_category . '", "' . $article_language . '", ' . $article_parent_id . ', "' . $article_slug . '", "' . date('Y-m-d H:i:s') . '")', true);
    } else {
        ...
        $response = sb_db_query('UPDATE sb_articles SET title = "' . $article_title . '", content = "' . $article_content . '", editor_js = "' . $article_editor_js . '", nav = "' . sb_db_json_escape($headings) . '", link = "' . $article_link . '", category = "' . $article_category . '", parent_category = "' . $article_parent_category . '", language = "' . $article_language . '", parent_id = ' . $article_parent_id . ', slug = "' . $article_slug . '", update_time = "' . date('Y-m-d H:i:s') . '" WHERE id = ' . $article_id);
    }
    return $response;
}
...
function sb_get_articles($article_id = false, $count = false, $full = false, $categories = false, $language = false, $skip_language = false) {
    ...
    $is_multilingual_by_translation = defined('SB_DIALOGFLOW') && $language && $language != 'all' && sb_get_multi_setting('google', 'google-multilingual-translation') && sb_get_multi_setting('articles-language-settings', 'articles-auto-translation');
    if (!$article_id) {
        $query_part .= ($query_part ? ' AND ' : '') . (empty($language) || $language == 'all' || $is_multilingual_by_translation ? '(parent_id IS NULL OR parent_id = "")' : 'language = "' . ($language ? sb_db_escape($language) : '') . '"');
    }
    ...
    if ($is_multilingual_by_translation) {
        for ($i = 0; $i < count($articles); $i++) {
            if ($articles[$i]['language'] != $language) {
                $article_translated = sb_db_get('SELECT * FROM sb_articles WHERE parent_id = "' . $articles[$i]['id'] . '" AND language = "' . sb_db_escape($language) . '"');
                if ($article_translated) {
                    $articles[$i] = $article_translated;
                } else {
                    $article = sb_google_translate_article($articles[$i]['id'], $language);
...
function sb_get_article_url($article) {
    ...
    return $url_rewrite ? $articles_page_url_slash . $cloud_url_part . sb_isset($article, 'slug', $article['id']) : $articles_page_url . '?article_id=' . $article['id'] . $cloud_url_part;
}
...
function sb_get_article_languages($main_article_id) {
    $main_article_id = sb_db_escape($main_article_id, true);
    $articles = sb_db_get('SELECT id, slug, language FROM sb_articles WHERE parent_id = ' . $main_article_id . ' OR id = ' . $main_article_id, false);
    foreach ($articles as &$article) {
        if (empty($article['language'])) {
            $article['language'] = sb_get_multi_setting('articles-language-settings', 'articles-default-language', 'en');
            break;
        }
    }
    return $articles;
