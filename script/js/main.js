                ...
                SBF.ajax({
                    ...
                    if (!admin && is_user && activeUser().type == 'visitor') {
                        ...
                        if (admin && SB_ADMIN_SETTINGS.slack_active) {
                            SBApps.slack.message(activeUser().id, SB_ACTIVE_AGENT.full_name, SB_ACTIVE_AGENT.profile_image, message, attachments);
        ...
        initChat: function () {
            ...
            SBF.getActiveUser(true, () => {
                ...
                if (CHAT_SETTINGS.header_name && active && user_type == 'user' && !tickets) {
                    chat_header.find('.sb-title').html(sb_(CHAT_SETTINGS.header_name).replace('{user_name}', activeUser().nameBeautified));
                }
                this.welcome();
                if (!SBPusher.active) {
                    setInterval(() => {
            ...
            if (!admin && !tickets && !this.dashboard && (this.agent_id == -1 || (this.conversation.getLastMessage() && SBF.isAgent(this.conversation.getLastMessage().get('user_type')) && this.conversation.getLastMessage().get('user_id') != this.agent_id))) {
                ...
                if (agent) {
                    ...
                    if (replace_agent_animation) {
                        ...
                        chat_header.html(`<div class="sb-dashboard-btn sb-icon-arrow-left"></div><div class="sb-profile"><img loading="lazy" src="${agent['profile_image']}" /><div><span class="sb-name">${agent['full_name']}</span><span class="sb-status">${sb_('Away')}</span></div><i class="sb-icon sb-icon-close ${CHAT_SETTINGS.close_chat ? 'sb-close-chat' : 'sb-responsive-close-btn'}"></i>${mobile ? '' : '<i class="sb-icon sb-icon-arrows-' + (main.hasClass('sb-chat-large') ? 'in' : 'out') + ' sb-enlarge-chat-widget"></i>'}</div><div class="sb-label-date-top"></div>`);
                    }
