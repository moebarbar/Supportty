 ...
 * ==========================================================
 *
 * Main Javascript admin file. © 2017-2026 board.support. All rights reserved.
 * 
 */
...
(function ($) {
    ...
    var chatbot_training_panel;
    var suggestions_area;
    var pagination_count = 1;
    var open_ai_button_rewrite;
    var editor_textarea;
    ...
    */
    ...
    $.fn.sbInitTooltips = function () {
        $(this).find('[data-sb-tooltip]').each(function () {
            let text = $(this).attr('data-sb-tooltip');
            $(this).miniTip({
                content: text,
                anchor: 's',
                delay: 500,
                className: text.length > 40 ? 'sb-tooltip-large' : ''
            });
        });
        return this;
    }
    ...
    function saved_reply_search(search) {
        SBF.search(search, () => {
            ...
            area.html(code);
            if (!all && SB_ADMIN_SETTINGS.chatbot_features) {
    ...
    function dialogDeleteFile(url, id, title) {
        window.open(url);
        infoPanel(`<span>${sb_('For security reasons, delete the file after downloading it. Close this window to automatically delete it. File location:')}</span><pre>${url}</pre>`, 'info', false, id, title);
    }
    ...
    var SBApps = {
            ...
            trainingPanel: {
                ...
                show: function (message_id) {
                    ...
                    SBF.ajax({
                        ...
                        chatbot_training_panel.find('#sb-qea-select').html(code);
                    });
                    SBApps.openAI.generateQuestions(question);
                    chatbot_training_panel.attr('data-message-id', message.id);
                    chatbot_training_panel.find('.sb-type-text:not(.sb-first)').remove();
                    chatbot_training_panel.find('.sb-type-text input').val(question);
                    chatbot_training_panel.find('#sb-qea-select').val('');
                    chatbot_training_panel.find('.sb-search-btn').sbActive(false).find('input').val('');
                    this.search('');
                    this.original_response = response;
                    chatbot_training_panel.find('textarea').val(response);
                    chatbot_training_panel.sbShowLightbox();
                },

                submit: function (button) {
                    if (loading(button)) return;
                    let questions = [];
                    let answer = chatbot_training_panel.find('textarea').val();
                    let existing_qea = chatbot_training_panel.find('#sb-qea-select').val();
                    chatbot_training_panel.find('.sb-type-text input').each(function () {
                        if ($(this).val()) {
                            questions.push($(this).val());
                        }
                    });
                    if ((!answer && !existing_qea) || questions.length == 0) {
                        SBForm.showErrorMessage(chatbot_training_panel, 'Enter an answer and at least one question.');
                        $(button).sbLoading(false);
                    } else {
                        let questions_answers;
                        if (existing_qea) {
                            this.qea[existing_qea][0] = this.qea[existing_qea][0].concat(questions);
                            questions_answers = this.qea[existing_qea];
                        } else {
                        ...
                        SBF.ajax({
                            ...
                            update_index: existing_qea,
                            language: chatbot_training_panel.find('#sb-chatbot-training-panel-language select').val()
                        }, (response) => {
                            $(button).sbLoading(false);
                            if (response === true) {
                                ...
                                SBForm.showErrorMessage(chatbot_training_panel, response.error && response.error.message ? response.error && response.error.message : 'Error');
                            }
                        });
                    }
                },
                ...
                search: function (search) {
                    ...
                    chatbot_training_panel.find('#sb-qea-select').html(code).change();
                    if (all) {
                        chatbot_training_panel.find('textarea').val(this.original_response);
                    }
                },
        ...
        openAI: {
            ...
            rewriteButton: function (value) {
                if (open_ai_button_rewrite.length) {
                    open_ai_button_rewrite.sbActive(value.length > 2 && value.indexOf(' '));
                }
            },

            rewrite: function (message, onSuccess) {
                SBF.ajax({
                    function: 'open-ai-rewrite-message',
                    message: message
                }, (response) => {
            ...
            getCode: {
                ...
                repeater_: function (title, code) {
                    return `<div class="sb-title">${sb_(title)}</div><div data-type="repeater" class="sb-setting sb-type-repeater"><div class="input"><div class="sb-repeater sb-repeater-block-${SBF.stringToSlug(title)}">${code}</div><div class="sb-btn sb-repeater-add sb-icon"><i class="sb-icon-plus"></i>${sb_('Add new item')}</div></div></div>`;
                }
            },

            generateQuestions: function (expression) {
                if (SB_ADMIN_SETTINGS.open_ai_user_expressions && expression) {
                    let loader = chatbot_training_panel.find('[data-value="add"]');
                    loader.sbLoading(true);
                    chatbot_training_panel.find('.sb-open-ai-intent').remove();
                    SBF.ajax({ function: 'open-ai-user-expressions', message: expression }, (response) => {
                        ...
                        if (code) chatbot_training_panel.find('> div > .sb-type-text').last().after(code);
                        loader.sbLoading(false);
                    });
                }
            },

            getAudioTranscriptCode: function (segments) {
                let code = `<div class="sb-title">${sb_('Transcript')}</div><div class="sb-audio-transcript-cnt">`;
                segments.forEach(segment => {
                    code += `<div data-speaker="${segment.speaker}"><span>${sb_('Speaker')} ${segment.speaker}</span><span>${segment.text}</span></div>`;
                });
                return code + '</div>';
            },
            ...
            train: {
                ...
                files: function (onSuccess, index = 0) {
                    ...
                    if (this.isFile(file_name) && !this.skip_files.includes(file_name)) {
                        admin.find('#sb-embeddings-box p').html(`<span>${sb_('We are processing the source')}</span><pre>${file_name}</pre><span>${sb_('Only {R} sources left to complete.').replace('{R}', files.length - index)}</span>`);
                ...
                website: function (onSuccess, index = 0) {
                    ...
                    if (url && url.includes('http')) {
                        admin.find('#sb-embeddings-box p').html(`<span>${sb_('We are processing the source')}<pre>${url}</pre><span>${sb_('Only {R} sources left to complete.').replace('{R}', this.urls.length - index)}</span>`);
                        if (url.includes('.xml')) {
                ...
                sitemap: function (url, onSuccess) {
                    admin.find('#sb-embeddings-box p').html(`<span>${sb_('We are generating the sitemap')}</span><pre>${url}</pre>`);
            ...
            init: function () {
                SBF.ajax({
                    ...
                    chatbot_files_table.sbLoading(false).find('tbody').html(code[1]);
                    chatbot_website_table.sbLoading(false).find('tbody').html(code[0]);
                    chatbot_area.find('#sb-chatbot-delete-files, #sb-table-chatbot-files').setClass('sb-hide', !code[1]);
                    chatbot_area.find('#sb-chatbot-delete-website, #sb-table-chatbot-website').setClass('sb-hide', !code[0]);
                });
                SBF.ajax({
                    function: 'open-ai-get-qea-training'
                }, (response) => {
        ...
        messenger: {
            ...
            sendComment: function (comment_id, facebook_page_id, message, message_id, is_instagram = true, onSuccess = false) {
                SBF.ajax({
                    function: 'messenger-send-comment',
    ...
    var SBSettings = {
        ...
        get: function (item) {
            ...
            switch (type) {
                ...
                case 'select-ajax':
                    value = item.find('input').attr('data-value');
                    return [id, [value ? value : '', item.find('input').val()], type];
                case 'select-checkbox':
                    return [id, [item.find('.sb-select-checkbox-input').val(), item.find('.sb-select-checkbox-input').attr('data-value')], type];
                case 'checkbox':
        ...
        set: function (id, setting) {
            ...
            switch (type) {
                ...
                case 'select':
                    ...
                    break;
                case 'select-ajax':
                    settings_area.find(`${id} input`).attr('data-value', value[0]).val(value[1]);
                    break;
                case 'select-checkbox':
                    let values = value && value[0] ? value[0].split(',') : [];
                    values.forEach((value_) => {
                        settings_area.find(`input[id="${value_.replace(/ /g, '')}"]`).prop('checked', true);
                    });
                    settings_area.find(id + ' .sb-select-checkbox-input').attr('data-value', value[0]).val(value[1]);
                    break;
                case 'checkbox':
        ...
        input: {
            set: function (input, value) {
                ...
                if (input.is('select')) {
                    ...
                    } else if (input.attr('data-type') == 'select-ajax') {
                        input.attr('data-value', value[0]).attr('value', typeof value[1] === ND ? value[0] : value[1]); // Deprecated. Replace  typeof value[1] === ND ? value[0] : value[1] with: value[1] 
                    } else if (input.attr('data-type') == 'select-checkbox') {
                        input.attr('data-value', value[0]).attr('value', value[1]);
            ...
            get: function (input) {
                ...
                if (input.is(':checkbox')) {
                    ...
                    return [input.attr('data-value'), input.val()];
                } else if (input.attr('data-type') == 'select-checkbox') {
                    return [input.attr('data-value'), input.val()];
        ...
        getDatePickerSettings: function (is_single = false, is_future_only = false, is_past_only = false) {
            let settings = {
                ranges: {},
                locale: {
                    format: 'DD/MM/YYYY',
        ...
        visibility: function (index, visible) {
            let selectors = [
                ['#push-notifications-onesignal-sw-url, #push-notifications-onesignal-app-id, #push-notifications-onesignal-api-key, #push-notifications-sw-path', '#push-notifications-id, #push-notifications-key'],
                ['#messenger-key, #messenger-path-btn', '#messenger-sync-btn'],
                ['#open-ai-assistant-id', '#open-ai-prompt,#open-ai-model, #open-ai-tokens, #open-ai-temperature, #open-ai-presence-penalty, #open-ai-frequency-penalty, #open-ai-logit-bias, #open-ai-custom-model, #open-ai-source-links'],
                [false, '#header-exclude-admins']
            ];
    ...
    */
    ...
    var SBReports = {
            ...
            this.charts.push(new Chart(canvas, {
                ...
                options: {
                    ...
                    scales: {
                        yAxes: [{
                            gridLines: {
                                display: type != 'pie' && type != 'horizontalBar',
                                drawBorder: false,
                                color: SBF.storage('dark-mode') ? '#444444' : '#ebebeb',
                                zeroLineColor: SBF.storage('dark-mode') ? '#444444' : '#ebebeb'
                            },
                            ticks: {
                                callback: function (tick_value) {
                                    return type != 'pie' ? (label_type == 1 ? tick_value : (label_type == 2 ? new Date(tick_value * 1000).toISOString().substr(11, 8) : tick_value)) : '';
                                },
                                beginAtZero: true
                            }
                        }],
                        ...
                        callbacks: {
                            label: function (tooltipItem, chartData) {
                                ...
                                switch (label_type) {
                                    ...
                                    case 3: return value + '%';
                                    case 4: return chartData.labels[index];
                                }
                            },
                        },
                        displayColors: false
        ...
        initReport: function (category_name = false, date_range = false) {
            let area = reports_area.find('> .sb-scroll-area');
            date_range = SBF.null(date_range) ? [false, false] : date_range.split(' - ');
            area.sbLoading(true);
            if (category_name) {
                this.active_report_category_name = category_name;
            }
            if (!this.active_report_category_name) {
                return;
            }
            this.active_date_range = date_range;
            SBF.ajax({
                function: 'reports-get-code',
                name: this.active_report_category_name,
                date_start: date_range[0],
                date_end: date_range[1],
                timezone: Intl.DateTimeFormat().resolvedOptions().timeZone
            }, (response) => {
                area.html(response.code).sbLoading(false);
                area.sbInitTooltips();
                this.charts.forEach((chart) => {
                    chart.destroy();
                });
                this.charts = [];
                this.active_reports = response.reports;
                for (let key in response.reports) {
                    this.initChart(key, response.reports[key].data, response.reports[key].chart_type, response.reports[key].label_type);
                }
                SBF.event('SBReportsLoaded', { report_category_name: this.active_report_category_name, date_range: date_range });
            });
        },

        export: function (report_name, onSuccess) {
            SBF.ajax({
        ...
        open: function (category_name) {
            header.find('.sb-admin-nav #sb-reports').click();
            this.pending_open = category_name;
        },

        openSingle: function (report_name) {
            const report = this.active_reports[report_name];
            let code = '<div class="sb-chart" data-report-name="' + report_name + '-panel"><canvas></canvas></div><table class="sb-table"><thead><tr>';
            let index = report.data[Object.keys(report.data)[0]].length - 1;
            let list = [];
            for (var h of report.table) {
                code += `<th>${h}</th>`;
            }
            code += '</tr></thead><tbody>';
            for (var key in report.data) {

                list.push([key, report.data[key][index]]);

            }
            if (report.table_inverse) {
                list.reverse();
            }
            ...
            code += `</tbody></table>`;
            if (report.average) {
                code += `<div class="sb-report-total"><span>${report.average_label}</span><span>${report.average}</span></div>`;
            }
            SBAdmin.genericPanel('report', report.title, code, [['Export', 'file']], 'data-report-name="' + report_name + '"', true);
            this.initChart(report_name + '-panel', report.data, report.chart_type, report.label_type);
        }
    }
    ...
    */
    ...
    var SBConversations = {
        ...
        openConversation: function (conversation_id, user_id = false, scroll = true) {
            ...
            if (user_id === false && conversation_id) {
                ...
                if (active_conversation.length) {
                    ...
                    SBF.storage('message-drafts', this.message_drafts);
                }
                editor_textarea.val('');
                ...
                if (SBPusher.active) {
                    ...
                    SBPusher.event('new-message', (response) => {
                        if (response && response.action && ['message_delete', 'message_update'].includes(response.action)) {
                            SBChat.id_last_message_conversation = response.message_id - 2;
                            setTimeout(() => {
                                SBChat.update();
                            }, 1000);
                        } else {
                        ...
                        if (response) {
                            if (response.terminate_call || response.incoming_call || response.call_response) {
                                SBF.call.show(response.incoming_call || response.call_response ? true : false, response.call_response && response.call_response.status == 'RINGING');
                                if (response.terminate_call) {
                                    delete this.active_calls[response.conversation_id];
                                    SBF.call.end();
                                } else {
                                    this.active_calls[response.conversation_id] = response.call_response ? response.call_response : response.incoming_call;
                                    if (response.call_response && response.call_response.status == 'ACCEPTED') {
                                        SBF.call.startCall();
                                    }
                                }
                            } else if (response.connect_call) {
                                if (SBF.call.rtc) {
                                    SBF.call.rtc.setRemoteDescription({
                                        type: 'answer',
                                        sdp: response.connect_call.session.sdp
                                    });
                                    profile_box.find('[data-value="whatsapp-call"]').sbLoading(false);
                                    SBConversations.open(response.conversation_id);
                                }
                            }
                        }
                    });
                ...
                if (conversation_id != -1) {
                    ...
                    activeUser().getFullConversation(conversation_id, (response) => {
                        ...
                        conversations_area.find('.sb-conversation-busy').remove();
                        this.updateUserDetails();
                        conversations_area.find('.sb-top > a').html(SBF.getFileType(title) == 'image' ? `<img src="${title}" />${title.includes('t=') ? ` <span>${new URL(title).searchParams.get('t')}</span>` : ''}` : title);
                        ...
                        // Source tasks
                        let last_user_message = response.getLastUserMessage();
                        let active_call = ['connect_call', 'call_response', 'incoming_call'].includes(response.messages[response.messages.length - 1].payload('event')) ? response.messages[response.messages.length - 1].payload() : false;
                        if (response.get('source') == 'em') {
                            this.cc(response.get('extra').split(','));
                        }
                        if (last_user_message && response.get('source') == 'wa') {
                            if ((new Date()) - (new Date(last_user_message.get('creation_time').replace(' ', 'T'))) > 86400000) {
                        ...
                        SBF.call.show(active_call ? true : false, active_call && active_call.response === 'RINGING');
                        if (active_call) {
                            this.active_calls[conversation_id] = { id: active_call.call_id, session: { sdp: active_call.sdp }, phone_number_id: active_call.phone_number_id };
                        }
        ...
        getInfoMessage(message) {
            ...
            let agent_name = message.payload('agent_name');
            if (event) {
                if (agent_name) {
                    let value = event.split('-').pop();
                    if (SB_ACTIVE_AGENT.full_name == agent_name) {
                        agent_name = sb_('You');
                    }
                    if (event.includes('conversation-department-update')) {
                        return sb_('{R} assigned the conversation to the {R2} department.').replace('{R}', `<b>${agent_name}</b>`).replace('{R2}', `<b>${this.getDepartments(value) ? this.getDepartments(value)['department-name'] : value}</b>`);
                    } else if (event.includes('conversation-agent-update')) {
                        return sb_('{R} assigned the conversation to {R2}.').replace('{R}', `<b>${agent_name}</b>`).replace('{R2}', `<b>${message.payload('agent_name_assigned')}</b>`);
                    } else if (event.includes('conversation-status-update') && (value == 3 || value == 4)) {
                        return sb_('{R} {R2} the conversation.').replace('{R}', `<b>${agent_name}</b>`).replace('{R2}', `<b>${sb_(value == 3 ? 'archived' : 'deleted')}</b>`);
                    }
                } else if (event == 'incoming_call') {
                    return '<i class="sb-icon-phone-call-1"></i>' + sb_('Incoming voice call');
                } else if (event == 'terminate_call') {
                    const duration = message.payload('duration');
                    return `<span class="sb-info-message-btn" data-id="call"><i class="sb-icon-phone-call-${duration ? 1 : 2}"></i>${duration ? `${sb_('Voice call')} (${SBF.getMinutesSeconds(duration)}) </span>` : sb_('Call not answered')}`;
                } else if (event == 'call_permission_request') {
                    return `${sb_('Call permission request')} ${sb_(message.payload('response') == 'accept' ? 'accepted' : 'rejected')}`;
                } else if (event == 'call_response') {
                    return `<i class="sb-icon-phone-call-${['RINGING', 'ACCEPTED'].includes(message.payload('response')) ? 1 : 2}"></i>${sb_('Call')} ${sb_(message.payload('response').toLowerCase())}`;
                }
            }
            return '';
        },
        ...
        update: function () {
            ...
            if (conversations_filters.eq(0).find('p').attr('data-value') == 0 && this.datetime_last_conversation && active_admin_area == 'conversations') {
                        ...
                        for (var i = 0; i < response.length; i++) {
                            if (!id_check.includes(response[i].id)) {
                                ...
                                if (active_conversation) {
                                    ...
                                    let conversation_ = conversations[conversation_li.index()];
                                    if (conversation_ && conversation instanceof SBConversation && conversation.messages.length == 1 && !conversation.messages[0].message && !conversation.messages[0].attachments.length) {
                                        conversation.messages[0].set('message', conversation_.messages[0].message);
                                        conversation.messages[0].set('attachments', conversation_.messages[0].attachments);
                                    }
        ...
        updateUserDetails() {
            ...
            SBProfile.populate(activeUser(), conversations_area.find('.sb-panel-conversation-details'));
        },
        ...
        attachments: function () {
            if (attachments_panel.length) {
                ...
                for (var i = attachments.length - 1; i > -1; i--) {
                    ...
                    code += `<a href="${attachments[i][1]}" target="_blank"><img src="${SB_URL}/media/files/${SBF.getFileIcon(attachments[i][1])}.svg">${attachments[i][0]}</a>`;
    ...
    */
    ...
    var SBProfile = {
            ...
            activeUser().update((response) => {
                ...
                activeUser().getConversations((response) => {
                    ...
                    if (!SBF.null(activeUser().get('email'))) {
                        profile_box.find('.sb-top-bar [data-value="custom_email"]').sbActive(true);
                    }
                    if (activeUser().getExtra('phone')) {
                        if (SB_ADMIN_SETTINGS.sms) {
                            profile_box.find('.sb-top-bar [data-value="sms"]').sbActive(true);
                        }
                        profile_box.find('.sb-top-bar [data-value="whatsapp"],.sb-top-bar [data-value="whatsapp-call"]').sbActive(true);
                    }
                    this.boxClasses(profile_box, user_type);
                    profile_box.attr('data-user-id', activeUser().id).sbShowLightbox();
                    profile_box.find('.sb-start-conversation').setClass('sb-hide', response.length == 1 && response[0].get('extra_3') == 'call');
        ...
        // Populate profile
        populate: function (user, profile_area, is_extra = true) {
            let exclude = ['first_name', 'last_name', 'password', 'profile_image'];
            let code = ['', '', ''];
            if (profile_area.hasClass('sb-panel-conversation-details') && SBChat.conversation) {
                ...
                if (!SBF.null(source)) {
                    let label = SBApps.getMessagingAppName(source);
                    let extra = SBChat.conversation.get(source == 'tg' ? 'extra_3' : 'extra');
                    let extra_2 = false;
                    let extra_3 = SBChat.conversation.get('extra_3');
                    if (extra_3) {
                        if (source == 'ig') {
                            label = 'Instagram' + ' ' + sb_('comment');
                            extra_2 = JSON.parse(extra_3)[0];
                        } else if (source == 'fb') {
                            label = 'Facebook' + ' ' + sb_('comment');
                            extra_2 = extra_3;
                        }
                    } else if (SB_ADMIN_SETTINGS.channel_labels[extra]) {
                        label = SB_ADMIN_SETTINGS.channel_labels[extra];
                    }
                    code[1] += this.profileRow('conversation-source', label, sb_('Source'), SBApps.getMessagingAppName(source), extra_2);
                }
            ...
            if (user.isExtraEmpty() && is_extra) {
                ...
                let is_wrap = profile_area.hasClass('sb-profile-list');
                profile_area.html(`${is_wrap ? '' : '<div class="sb-profile-list">'}<ul class="ul-user">${code[0]}</ul>${code[1] ? `<div class="sb-title">${sb_('Conversation')}</div><ul class="ul-conversation">${code[1]}</ul>` : ''}${code[2] ? `<div class="sb-title">${sb_('Technology')}</div><ul class="ul-technology">${code[2]}</ul></div>${is_wrap ? '' : '</div>'}` : ''}`);
                collapse(profile_area, 145);
                SBF.event('SBUserLoaded', user);
            }
        },

        profileRow: function (key, value, name = key, extra = false, extra_2 = false) {
            if (!value) return '';
            ...
            switch (key) {
                ...
                case 'conversation-source':
                    image = `apps/${(extra ? extra : value).toLowerCase()}.svg`;
                    break;
                case 'location':
            ...
            return `<li data-id="${key}"${extra ? ` data-extra="${extra}"` : ''}${extra_2 ? ` data-extra-2="${extra_2}"` : ''}>${image ? `<img src="${SB_URL}/media/${image}" />` : icon}<span>${sb_(SBF.slugToString(name))}</span><label>${value}</label></li>`;
        },
    ...
    */
    ...
    var SBAdmin = {
        ...
        infoPanel: function (text, type = 'info', onConfirm = false, id = '', title = '', scroll = false, skip = false, onCancel = false) {
            ...
            box.find('.sb-title').html(title && title.includes('icon-') ? `<i class="sb-${title}"></i>` : title ? sb_(title) : (type == 'alert' ? '<i class="sb-icon-info"></i>' : ''));
        ...
        collapse(target, max_height) {
            ...
            if (target.hasClass('sb-collapse')) {
                if ($(content).prop('scrollHeight') > max_height) {
    ...
    */
    ...
    $(document).ready(function () {
        ...
        chatbot_training_panel = admin.find('.sb-dialogflow-intent-box');
        suggestions_area = conversations_area.find('.sb-editor > .sb-suggestions');
        open_ai_button_rewrite = conversations_area.find('.sb-btn-open-ai');
        ...
        // Dark & Light mode
        if (SBF.storage('dark-mode')) {
            SBF.loadResource(SB_URL + '/css/dark.css');
        }
        $(header).find('.sb-menu li:last-child, .sb-mobile li:last-child').before(`<li data-value="${SBF.storage('dark-mode') ? 'light' : 'dark'}-mode">${sb_(SBF.storage('dark-mode') ? 'Light mode' : 'Dark mode')}</li>`);

        $(header).on('click', '[data-value="dark-mode"], [data-value="light-mode"]', function () {
            let is_dark_mode = $(this).attr('data-value') == 'dark-mode';
            if (is_dark_mode) {
                SBF.loadResource(SB_URL + '/css/dark.css');
            } else {
                document.getElementById('sb-darkcss').remove();
            }
            SBF.storage('dark-mode', is_dark_mode);
            $(this).attr('data-value', is_dark_mode ? 'light-mode' : 'dark-mode').html(sb_(is_dark_mode ? 'Light mode' : 'Dark mode'));
        });
        ...
        $(window).keydown(function (e) {
            ...
            if ([13, 27, 32, 37, 38, 39, 40, 46, 90].includes(code)) {
                if (admin.find('.sb-dialog-box').sbActive()) {
                    ...
                    let is_editor_focus = editor_textarea.is(':focus');
                    if (code == 46) {
        ...
        // Autosave drafts
        setInterval(() => {
            if (active_admin_area == 'conversations' && SBChat.conversation) {
                let text = editor_textarea.val().trim();
                if (SBConversations.message_drafts[SBChat.conversation.id] != text) {
                    SBConversations.message_drafts[SBChat.conversation.id] = text;
        ...
        */
        ...
        if (responsive) {
            ...
            $(admin).find('.sb-admin-list .sb-scroll-area, main > div > .sb-scroll-area, .sb-area-settings > .sb-tab > .sb-scroll-area, .sb-area-reports > .sb-tab > .sb-scroll-area, [data-id="playground"], .sb-area-chatbot>.sb-tab').on('scroll', function () {
            ...
            $(admin).on('click', '#sb-close-filters-btn', function () {
                scrolls.always_hidden = false;
                admin.removeClass('sb-header-hidden');
            });
            ...
            $(admin).on('click', '.sb-playground-history .sb-close', function () {
                        ...
                        SBF.ajax({
                            ...
                            if (response) {
                                ...
                                if (response['slack-agents']) {
                                    let code = '';
                                    for (const key in response['slack-agents'][0]) {
                                        code += `<div id="${key}" data-type="select" class="multi-input-select"><select><option value="${response['slack-agents'][0][key][0]}" selected></option></select></div>`;
                                    }
                            ...
                            SBSettings.visibility(3, response['header-type'] ? response['header-type'][0] == 'agents' : true);
                    ...
                    if (reports_area.sbLoading()) {
                        SBSettings.loadDatePicker(() => {
                            $.getScript(SB_URL + '/vendor/chart.min.js', () => {
                                reports_area.find('#sb-date-picker').daterangepicker(SBSettings.getDatePickerSettings(false, false, true)).val(moment().subtract(29, 'days').format('DD-MM-YYYY') + ' - ' + moment().format('DD-MM-YYYY'));
                                $('.ranges li').eq(3).click();
        ...
        */
        ...
        $(admin).on('click', '.sb-btn-open-ai', function () {
            ...
            let textarea = is_editor ? editor_textarea : chatbot_training_panel.find('textarea');
            SBApps.openAI.rewrite(textarea.val(), (response) => {
                $(this).sbLoading(false);
                textarea.val(is_editor ? '' : response);
                if (is_editor) {
                    SBChat.insertText(response);
                }
            });
        });
        ...
        $(document).on('SBMessageSent', function (e, response) {
            ...
            if (SBApps.messenger.check(conversation)) {
                let last_user_message = conversation.getLastUserMessage();
                if (last_user_message && last_user_message.payload() && last_user_message.payload().cid) {
                    SBApps.messenger.sendComment(last_user_message.payload().cid, conversation.get('extra'), response.message, response.message_id, conversation.get('source') == 'ig', (response) => {
                        for (var i = 0; i < response.length; i++) {
        ...
        $(admin).on('click', '.sb-panel-conversation-details li', function () {
            ...
            switch ($(this).attr('data-id')) {
                ...
                case 'current_url':
                    ...
                    break;
                case 'conversation-source':
                    let source = ($(this).attr('data-extra') ? $(this).attr('data-extra') : label_value).toLowerCase();
                    let extra = $(this).attr('data-extra-2');
                    if (source == 'whatsapp' && activeUser().getExtra('phone')) {
                        window.open('https://wa.me/' + SBApps.whatsapp.activeUserPhone());
                    } else if (source == 'facebook') {
                        window.open('https://www.facebook.com/' + (extra ? extra : 'messages/t/' + SBChat.conversation.get('extra')));
                    } else if (source == 'instagram') {
                        window.open('https://www.instagram.com/' + (extra ? extra : 'direct/inbox/'));
                ...
                case 'cc':
                    if (SBChat.conversation && SBChat.conversation.get('source') == 'em') {
                        ...
                        code += `</div><div class="sb-btn sb-repeater-add sb-icon"><i class="sb-icon-plus"></i>${sb_('Add new item')}</div></div></div>`;
        ...
        // Dialogflow
        $(chatbot_training_panel).on('click', '.sb-intent-add [data-value="add"]', function () {
            chatbot_training_panel.find('> div > .sb-type-text').last().after('<div class="sb-setting sb-type-text"><input type="text"></div>');
        });

        $(chatbot_training_panel).on('click', '.sb-intent-add [data-value="previous"],.sb-intent-add [data-value="next"]', function () {
            let input = chatbot_training_panel.find('.sb-first input');
            ...
            for (var i = 0; i < messages_length; i++) {
                if (messages[i].message == message && ((next && i < (messages_length - 1)) || (!next && i > 0))) {
                    ...
                    chatbot_training_panel.attr('data-message-id', messages[i].id);
        ...
        $(chatbot_training_panel).on('click', '.sb-send', function () {
            SBApps.dialogflow.trainingPanel.submit(this);
        });

        $(chatbot_training_panel).on('input', '.sb-search-btn input', function () {
            SBApps.dialogflow.trainingPanel.search($(this).val());
        });

        $(chatbot_training_panel).on('click', '.sb-search-btn i', function () {
            SBF.searchClear(this, () => { SBApps.dialogflow.trainingPanel.search($(this).val()) });
        });

        $(chatbot_training_panel).on('click', '#sb-qea-preview', function () {
            SBApps.dialogflow.trainingPanel.preview(chatbot_training_panel.find('#sb-qea-select').val());
        });

        $(chatbot_training_panel).on('change', '#sb-qea-select', function () {
            let qea = $(this).val();
            chatbot_training_panel.find('.sb-bot-response').setClass('sb-disabled', qea).find('textarea').val(qea ? SBApps.dialogflow.trainingPanel.qea[qea][1] : SBApps.dialogflow.trainingPanel.original_response);
        });

        $(chatbot_training_panel).on('change', 'textarea', function () {
            clearTimeout(timeout);
            timeout = setTimeout(() => {
                SBApps.dialogflow.trainingPanel.original_response = chatbot_training_panel.find('textarea').val();
            }, 500);
        });
        ...
        notes_panel.on('click', '.sb-delete-note', function () {
            ...
            SBConversations.notes.delete(SBChat.conversation.id, item.attr('data-id'), () => {
                item.remove();
            });
        });
        ...
        $(conversations_area).on('click', '.sb-list .sb-menu > li', function () {
            ...
            switch (value) {
                ...
                case 'original':
                    ...
                    break;
                case 'bot':
                    SBApps.dialogflow.trainingPanel.show($(this).closest('[data-id]').attr('data-id'));
                    break;
                case 'reply':
        ...
        $(conversations_area).on('click', '#sb-close-filters-btn', function () {
            conversations_area.find('.sb-filter-btn').sbActive(false);
        });
        ...
        // Info messages
        $(conversations_area).on('click', '.sb-info-message-btn', function () {
            let message_id = $(this).closest('.sb-info-message').attr('data-id');
            let message = SBChat.conversation.getMessage(message_id);
            let code = '';
            let title = '';
            let buttons = [];
            let attributes = ' data-message-id="' + message_id + '"';
            switch ($(this).attr('data-id')) {
                case 'call':
                    title = 'WhatsApp Voice Call';
                    code = `<div class="sb-player"><div class="sb-player-btn sb-icon-play"></div><div class="sb-player-speed"><div class="sb-player-speed-number">1</div><div class="sb-icon-close"></div></div><div class="sb-player-time"><span>0:00</span> / <span>${SBF.getMinutesSeconds(message.payload('duration'))}</span></div><div class="sb-player-download sb-icon-arrow-down"></div><audio><source src="${message.payload('call_audio')}" type="audio/mpeg"></audio></div>`;
                    if (message.payload('transcript')) {
                        code += SBApps.openAI.getAudioTranscriptCode(message.payload('transcript'));
                    } else if (SB_ADMIN_SETTINGS.open_ai_chatbot_status === true) {
                        buttons = [['Generate transcript']];
                    }
                    break;
            }
            SBAdmin.genericPanel('call-info', title, code, buttons, attributes, true);
        });

        // Audio player
        $(admin).on('click', '.sb-lightbox .sb-player-btn', function () {
            SBF.audio.play(this, $(this).parent());
        });

        $(admin).on('click', '.sb-lightbox .sb-player-speed', function () {
            SBF.audio.speed(this);
        });

        $(admin).on('click', '.sb-lightbox .sb-player-download', function () {
            SBF.audio.download(this);
        });

        $(admin).on('click', '#sb-generate-transcript', function () {
            if (loading(this)) return;
            let lightbox = $(this).closest('.sb-lightbox');
            const message_id = lightbox.attr('data-message-id');
            SBF.ajax({
                function: 'open-ai-audio-transcript',
                url: SBChat.conversation.getMessage(message_id).payload('call_audio'),
                message_id: message_id
            }, (response) => {
                $(this).sbLoading(false);
                lightbox.find('.sb-main').append(SBApps.openAI.getAudioTranscriptCode(response.segments));
            });
        });
        ...
        */
        ...
        $(users_table).on('click', ':checkbox', function () {
            ...
            if (users_table.find('td input:checked').length == 2 && users_table_menu.find('.sb-active').attr('data-type') != 'agent') {
        ...
        // User profile top bar actions
        $(profile_box).on('click', '.sb-top-bar [data-value]', function () {
            let value = $(this).attr('data-value');
            if (value == 'whatsapp-call') {
                if (loading(this)) return;
                SBF.call.start('whatsapp', profile_box.attr('data-user-id'), () => {
                    $(this).sbLoading(false);
                });
            } else {
                SBConversations.showDirectMessageBox(value, [activeUser().id]);
            }
        });
        ...
        */
        ...
        $(admin).on('click', '.sb-repeater-upload, [data-type="upload-image"] .image, [data-type="upload-file"] .sb-btn, #sb-chatbot-add-files, #sb-import-settings a, #sb-import-users a', function () {
            ...
            if ($(this).attr('id') == 'sb-chatbot-add-files') {
                ...
                upload_function = function () {
                    ...
                    chatbot_files_table.append(code).removeClass('sb-hide');
                };
        ...
        $(settings_area).on('click', '.sb-select-checkbox-input', function () {
            let value = $(this).attr('data-value');
            if (value) {
                value.replace(/ /g, '').split(',').forEach((id) => {
                    $(this).next().find('#' + id).prop('checked', true);
                });
            }
            $(this).toggleClass('sb-active');
        });
        ...
        $(settings_area).on('click', '.sb-select-checkbox input', function () {
            let checked_value_ids = '';
            let checked_value_names = '';
            $(this).parent().siblings().addBack().each(function () {
                if ($(this).find('input').prop('checked')) {
                    checked_value_ids += $(this).find('input').attr('id') + ', ';
                    checked_value_names += $(this).find('label').html() + ', ';
                }
            });
            $(this).closest('.sb-select-checkbox').prev().attr('data-value', checked_value_ids.substring(0, checked_value_ids.length - 2)).val(checked_value_names.substring(0, checked_value_names.length - 2));
        });

        // AJAX select
        $(settings_area).on('click', '[data-type="select-ajax"]', function () {
            let keys = ['posts', 'messenger-get-posts'];
            let id = $(this).data('id');
            let input = $(this);
            if (!id) {
                id = $(this).attr('id');
            ...
            if (id != 'messenger-dm-comment-post-id') {
                keys = ['departments', 'get-departments'];
            }
            if (!input.is('input')) {
                input = input.find('input');
            }
            if (input.next().hasClass('sb-custom-select')) {
                return input.next().remove();
            }
            if (custom_selects[keys[0]]) {
                input.after(custom_selects[keys[0]]);
            } else {
                input.after('<div class="sb-custom-select sb-loading"></div>');
                SBF.ajax({
                    function: keys[1]
                }, (response) => {
                    let code = '<span data-value=""></span>';
                    let options = [];
                    if (id != 'messenger-dm-comment-post-id') {
                        options = Object.entries(response).map(([key, value]) => [key, value.name]);
                    } else {
                        options = response.map(item => [item.id, item.message]);
                    }
                    options.forEach((option) => {
                        code += `<span data-value="${option[0]}">${option[1]}</span>`;
                    });
                    custom_selects[keys[0]] = '<div class="sb-custom-select">' + code + '</div>';
                    input.next().remove();
                    input.after(custom_selects[keys[0]]);
                    input.sbLoading(false);
                    if (!options.length && keys[0] == 'departments') {
                        infoBottom('No departments found. Create departments from Settings > Miscellaneous > Departments.', 'info');
                    }
                });
            }
        });

        $(settings_area).on('click', '.sb-custom-select:not(.sb-ignore) > span', function (e) {
            $(this).parent().prev().attr('data-value', $(this).attr('data-value')).val($(this).html());
            $(this).parent().remove();
            e.preventDefault();
            return false;
        });
        ...
        $(settings_area).on('click', '[data-id="email-piping-tags"], [data-id="messenger-page-tags"], [data-id="whatsapp-cloud-numbers-tags"], [data-id="telegram-numbers-tags"], [data-id="line-tags"], #zalo-tags > input', function () {
            let select = $(this).next();
            let function_names = { tags: 'get-tags' };
            let id = $(this).data('id');
            if (!id) {
                id = $(this).attr('id');
            }
            ...
            if (!select.html()) {
                ...
                SBF.ajax({
                    function: function_names[id]
                }, (response) => {
                    let code = '';
                    let options = [];
                    let checked_values = $(this).attr('data-value');
                    if (checked_values) {
                        checked_values = checked_values.replace(/ /g, '').split(',');
                    } else {
                        checked_values = [];
                    }
                    if (id == 'tags') {
                        options = response.map(item => [item['tag-name'], item['tag-name']]);
                    }
                    options.forEach((option) => {
                        code += `<div class="multi-input-checkbox"><input id="${option[0]}" type="checkbox"${checked_values.includes(option[0].replace(/ /g, '')) ? ' checked' : ''}><label>${option[0]}</label></div>`;
                    });
                    select.html(code);
                    select.sbLoading(false);
                    if (!options.length && id == 'tags') {
                        infoBottom('No tags found. Create tags from Settings > Admin > Tags.', 'info');
                    }
                });
            }
        });
        ...
        $(settings_area).on('click', '.sb-timetable > div > div > div', function () {
            ...
            if (active) {
                ...
                $(this).append(`<div class="sb-custom-select sb-ignore">${select}</div>`).sbActive(true);
            }
        });

        $(settings_area).on('click', '.sb-timetable .sb-custom-select span', function () {
            $(this).closest('.sb-timetable').find('> div > div > .sb-active').html($(this).html()).attr('data-value', $(this).attr('data-value'));
            $(this).parent().sbActive(false);
        });
        ...
        // Slack  
        if (!SB_ADMIN_SETTINGS.cloud || !SB_ADMIN_SETTINGS.slack_client_id) {
            ...
            SBF.ajax({
                ...
                if (SBF.errorValidation(response, 'slack-token-not-found')) {
                    ...
                    response.agents.forEach(agent => {
                        select += `<option value="${agent.id}">${agent.name}</option>`;
                    });
                    response.slack_users.forEach(user => {
                        let existing_value = input.find(`#${user.id} select`).val();
                        code += `<div id="${user.id}" data-type="select" class="multi-input-select"><label>${user.name}</label><select>${select.replace(`value="${existing_value}">`, `value="${existing_value}" selected>`)}</select></div>`;
                    });
                }
                input.html(code);
                SBSettings.set('slack-agents', [response.saved, 'multi-input']);
            });
        });
        ...
        // Settings visibility
        $(settings_area).on('change', '.sb-type-select select,.multi-input-select select', function () {
            let value = $(this).val();
            switch ($(this).closest('[id]').attr('id')) {
                case 'open-ai-mode':
                    SBSettings.visibility(2, value != 'assistant');
                    break;
                case 'messenger-sync-mode':
                    SBSettings.visibility(1, value != 'manual');
                    break;
                case 'push-notifications-provider':
                    SBSettings.visibility(0, value == 'pusher');
                    SBF.ajax({ function: 'update-sw', url: value == 'pusher' ? 'https://js.pusher.com/beams/service-worker.js' : 'https://cdn.onesignal.com/sdks/web/v16/OneSignalSDK.sw.js' });
                    break;
                case 'header-type':
                    SBSettings.visibility(3, value == 'agents');
                    break;
        ...
        $(settings_area).on('click', '.sb-setting', function (e) {
            ...
            switch (id) {
                ...
                case 'google-multilingual-translation':
                    ...
                    break;
                case 'articles-auto-translation':
                    if ($(e.target).is(':checked') && !settings_area.find('#google-multilingual-translation input').is(':checked')) {
                        infoBottom('Enable Settings > Artificial Intelligence > Google > Multilingual via translation to use this feature.', 'info');
                    }
                    break;
                case 'open-ai-prompt':
                    infoBottom('Custom prompts may break the chatbot, configure them carefully.', 'info');
                    break;
                case 'open-ai-tokens':
                ...
                case 'google-calendar-slot-time':
                    ...
                    break;
                case 'dialogflow-human-takeover-disable-offline':
                case 'welcome-disable-office-hours':
                case 'follow-disable-office-hours':
                    if ((id == 'follow-disable-office-hours' && settings_area.find('#follow-disable-office-hours input').is(':checked')) || (id == 'welcome-disable-office-hours' && settings_area.find('#welcome-disable-office-hours input').is(':checked')) || (id == 'dialogflow-human-takeover-disable-offline' && settings_area.find('#dialogflow-human-takeover-disable-offline input').is(':checked'))) {
        ...
        */
        ...
        $(flows_area).on('click', '.sb-flow-block', function () {
            ...
            let code_repeater = `<div class="sb-title">{R}</div><div data-type="repeater" class="sb-setting sb-type-repeater"><div class="input"><div class="sb-repeater">{R2}</div><div class="sb-btn sb-repeater-add sb-icon"><i class="sb-icon-plus"></i>${sb_('Add new item')}</div></div></div>`;
            let code_conditions = `<div class="sb-title">${sb_('Conditions')}</div><div class="sb-flow-conditions"></div><div class="sb-add-condition sb-btn sb-icon"><i class="sb-icon-plus"></i>${sb_('Add condition')}</div>`;
        ...
        $(chatbot_area).on('click', '#sb-train-chatbot', function (e) {
            ...
            if (chatbot_area.find('.sb-menu-chatbot .sb-active').attr('data-type') == 'flows') {
                SBApps.openAI.flows.save(() => {
                    infoPanel(success_text, 'info', false, false, 'icon-check');
                });
                $(this).sbLoading(false);
                return;
            }
        ...
        $(chatbot_area).on('click', '#sb-table-chatbot-files td i, #sb-table-chatbot-website td i, #sb-chatbot-delete-files, #sb-chatbot-delete-website, #sb-chatbot-delete-all-training, #sb-chatbot-delete-all-training-conversations', function () {
            ...
            if (is_i && tr.hasClass('sb-pending')) {
                ...
                tr.remove();
                if (!chatbot_files_table.find('tbody tr').length) {
                    chatbot_files_table.addClass('sb-hide');
                }
                return;
            }
            infoPanel('The training data will be permanently deleted.', 'alert', () => {
                ...
                if (is_i) {
                    ...
                    if (!table.find('input:checked').length) {
                        table.find('tbody input').prop('checked', true);
                    }
        ...
        $(chatbot_area).on('click', '.sb-nav [data-value="info"]', function () {
            ...
            SBF.ajax({
                ...
                code += `</p><h2>${sb_('Total detected characters')}</h2><p>${response.total} ${sb_('chars') + (response.limit ? ' / ' + response.limit + ' ' + sb_('limit') : '')}</p><hr><div id="sb-chatbot-delete-all-training" class="sb-btn">${sb_('Delete all training data')}</div>`;
                area.html(code);
                area.sbLoading(false);
            });
        });
        ...
        $(chatbot_area).on('click', '#sb-table-chatbot-files th input, #sb-table-chatbot-website th input', function () {
            let is_checked = $(this).is(':checked');
            $(this).closest('table').find('tbody tr input').each(function () {
            ...
            if (message && !loading(this)) {
                ...
                SBF.ajax({
                    ...
                    if (response.conversation) {
                        ...
                        infoPanel('The playground is not working because multiple admin area instances are open. Close all others and keep only this one open.', 'alert');
                        console.error(response);
                    }
                    $(this).sbLoading(false);
                });
        ...
        $(chatbot_playground_editor).on('click', '[data-value="history"]', function () {
            ...
            chatbot_playground_history_panel.find('ul').html(code);
        ...
        */
        ...
        $(reports_area).on('click', '.sb-open-report', function () {
            SBReports.openSingle($(this).closest('[data-report-name]').attr('data-report-name'));
        });

        $(reports_area).on('click', '.sb-select-reports ul [data-value]', function () {
            let id = $(this).attr('data-value');
            SBReports.active_report_category_name = false;
            reports_area.attr('class', 'sb-area-reports sb-active sb-report-' + id);
            SBReports.initReport(id, reports_area.find('#sb-date-picker').val());
        ...
        $(admin).on('click', '#sb-export', function () {
            if (loading(this)) return;
            SBReports.export($(this).closest('[data-report-name]').attr('data-report-name'), (response) => {
                $(this).sbLoading(false);
                if (response) {
        ...
        if (SBF.getURL('report') && !reports_area.sbActive()) {
            header.find('.sb-admin-nav #sb-reports').click();
        }

        $(document).on('SBReportsLoaded', function () {
            let report_category_name = SBF.getURL('report') ? SBF.getURL('report') : SBReports.pending_open;
            if (report_category_name && SBReports.active_report_category_name != SBF.getURL('report')) {
                reports_area.find('[data-value="' + report_category_name + '"]').click();
                reports_area.find('.sb-select-reports ul').sbActive(false);
                SBReports.pending_open = false;
            }
        });
        ...
        */
        ...
        $(admin).on('click', '.sb-lightbox .sb-top-bar .sb-close', function () {
            admin.sbHideLightbox();
            if ($(this).closest('.sb-lightbox').parent().attr('id') == 'sb-generic-panel') {
                $(this).closest('.sb-lightbox').html('');
            }
        });
